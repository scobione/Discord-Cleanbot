...
loginBtn.addEventListener('click', () => {
    ...
    if (!key) { loginError.textContent = 'Bitte Key eingeben'; loginError.style.display = 'block'; return; }
...
async function fetchStatus() {
    try {
        ...
        connectionRetries++;
        statusDot.className = 'status-dot offline';
        connectionStatus.textContent = '⏳ Warte auf Server...';
    }
}
...
async function fetchServers() {
    ...
    try {
        ...
        if (!Array.isArray(servers) || servers.length === 0) {
            serverSelect.innerHTML = '<option value="">-- Keine Server --</option>';
            return;
        }
...
async function doReset(del) {
    ...
    try {
        ...
        const body = del ? { userKey, serverId: selectedServer } : { userKey, serverId: selectedServer, channelCount: parseInt(channelCount.value), channelName: channelName.value, channelMessage: channelMessage.value, messageRepeat: parseInt(messageCount.value) };
...
function showAdminPanel(keys) {
    ...
    panel.innerHTML = '<div class="admin-card">' +
        '<h2>👑 Admin-Menü</h2>' +
        '<h3 style="color:#a29bfe;margin-top:16px;">➕ Neuer Key</h3>' +
        '<input type="text" id="newKeyName" class="input" placeholder="Key-Name" style="margin-top:8px;">' +
        ...
        '<h3 style="color:#a29bfe;margin-top:16px;">📋 Keys (' + keys.length + ')</h3>' +
        '<div id="keyList">' + keys.map(k => '<div class="key-item"><div class="key-info"><strong>' + k.key + '</strong>Resets: ' + k.remainingResets + ' | ' + new Date(k.created).toLocaleDateString('de-DE') + '</div><div class="key-actions"><button class="btn-refill" data-key="' + k.key + '">+1</button><button class="btn-delete" data-key="' + k.key + '">🗑️</button></div></div>').join('') + '</div>' +
        '<button id="closeAdmin" class="btn btn-secondary" style="margin-top:16px;">Schließen</button></div>';
    ...
    document.querySelectorAll('.btn-refill').forEach(btn => {
        btn.addEventListener('click', async () => {
            await fetch(API_BASE + '/admin/refill-key', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ password: adminPassword, key: btn.dataset.key, amount: 1 }) });
            loadAdminPanel();
        });
    });
    ...
    document.querySelectorAll('.btn-delete').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            await fetch(API_BASE + '/admin/delete-key', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ password: adminPassword, key: btn.dataset.key }) });
            loadAdminPanel();
        });
    });
}
